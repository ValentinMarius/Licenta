// lib/app/features/onboarding/presentation/screens/journey_stage_screen.dart
// Captures how far along the user is on their journey.
// Passes the persisted onboarding state into the time commitment step.
// RELEVANT FILES:lib/app/features/onboarding/presentation/screens/time_commitment_screen.dart,lib/app/features/onboarding/state/onboarding_state.dart,lib/app/features/onboarding/data/onboarding_storage.dart
...
class JourneyStageScreen extends StatefulWidget {
  final double previousProgress;
  final double targetProgress;
  final OnboardingState initialState;

  const JourneyStageScreen({
    super.key,
    this.previousProgress = 0.25,
    this.targetProgress = 0.5,
    this.initialState = const OnboardingState.initial(),
  });

  @override
  State<JourneyStageScreen> createState() => _JourneyStageScreenState();
}

class _JourneyStageScreenState extends State<JourneyStageScreen> {
  ...
  void initState() {
    ...
    if (_flowState.journeyStage != OnboardingState.skipValue) {
      _selectedStage = _flowState.journeyStage;
    }
  ...
  Future<void> _persistAndNavigate(String stageValue) async {
    final updated = _flowState.copyWith(journeyStage: stageValue);
    setState(() {
      _flowState = updated;
    });
    await OnboardingStorage.saveState(updated);
    if (!mounted) return;
    _navigateToTimeCommitment(updated);
  }

  void _navigateToTimeCommitment(OnboardingState state) {
    Navigator.of(context)
        .push(
          AppRoutes.onboardingRoute(
            builder: (_) => TimeCommitmentScreen(
